...
func NewRabbitMQStore(username string, password string, addr string, queueName string) *RabbitMQStore {
	...
	// rabbit's healthcheck can pass slightly before it accepts AMQP connections,
	// so retry instead of dying on the first failed dial
	...
	for attempt := 1; attempt <= 12; attempt++ {
		...
		log.Printf("RabbitMQ not ready (attempt %d/12), retrying in 5s: %v\n", attempt, err)
