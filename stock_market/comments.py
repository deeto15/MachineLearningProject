...
async def hottest_new_comments(reddit_client, comment_limit: int):
    ...
    hottest_submission_generator = subreddit.search("What Are Your Moves Tomorrow", limit=1)
...
def get_gpt_parameters(comment_text: str) -> dict:
    return {
        ...
        'messages': [
            {
                'role': 'system',
                'content': comment_stock_info_prompt
            },
            {
                'role': 'user',
...
async def get_comment_stock_info(gpt_client, comment):
    response = await asyncio.to_thread(gpt_client.chat.completions.create, **get_gpt_parameters(comment.body))
...
def company_info_from_comments(comment_results: list[dict]):
    ...
    for comment in comment_results:
        ...
        if ticker in company_results:
            ...
            appendable_attributes = ['userPosition', 'header', 'priceDate', 'predictedDate', 'formattedPrice', 'body', 'time']
