...
def get_static_mappings(base_datetime):
    ...
    "thanksgiving": (datetime(base_datetime.year, 11, 1) + relativedelta(weekday=TH(+4))),  # 4th Thursday of November
    "memorial day": (datetime(base_datetime.year, 5, 31) + relativedelta(weekday=0, days=-7)),  # Last Monday in May
    "labor day": datetime(base_datetime.year, 9, 1) + relativedelta(weekday=MO(+1)),  # 1st Monday in September
    }
    ...
    list_of_keywords = {
        ...
        "eom": (base_datetime + relativedelta(day=31)).strftime('%m/%d/%Y'),  # End of the month
    ...
    list_of_weekdays = {
        ...
        (base_datetime + timedelta(days=i)).strftime('%m/%d/%Y')
        for i in range(7)
    }
